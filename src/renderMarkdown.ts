...
function profileSummary(record: RunRecord): string {
  const profile = record.project_profile;
  const critical = profile.critical_paths_matched?.length ? profile.critical_paths_matched.map((file) => `\`${file}\``).join(", ") : "none";
...
function renderAgentHandoffBase(record: RunRecord): string {
...
function renderQualityReviewBase(record: RunRecord): string {
...
export function renderQaReport(record: RunRecord): string {
  const profile = record.project_profile;
  const preferred = profile.preferred_validation ?? [];
  const notRun = preferred.filter((command) => !record.validation.commands_run.includes(command));
  const sections: string[] = [];
  if (preferred.length) sections.push(`### Preferred validation\n\n${list(preferred)}`);
  if (notRun.length) sections.push(`### Preferred validation not run\n\n${list(notRun)}`);
  if (profile.critical_paths_matched?.length) sections.push(`### Critical paths changed\n\n${list(profile.critical_paths_matched.map((file) => `\`${file}\``))}`);
  if (profile.ignored_paths_applied?.length) sections.push(`### Ignored paths applied\n\n${list(profile.ignored_paths_applied.map((file) => `\`${file}\``))}`);
  if (profile.warnings.length) sections.push(`### Profile warnings\n\n${list(profile.warnings)}`);
  const context = sections.length ? `## Project Profile Evidence\n\n${sections.join("\n\n")}\n\n` : "";
  return renderQaReportBase(record)
    .replace(
      "## Overall Readiness\n\n",
      `## Overall Readiness\n\n- Run ID: ${record.run_id}\n- Created at: ${record.created_at}\n- Readiness: `,
    )
...
export function renderAgentHandoff(record: RunRecord): string {
  const profile = record.project_profile;
  const sections: string[] = [`- Project: ${profile.project_name || "not specified"}\n- Description: ${profile.description || "not specified"}`];
  if (profile.review_focus?.length) sections.push(`### User-defined review focus\n\n${list(profile.review_focus)}`);
  if (profile.notes?.length) sections.push(`### Profile notes\n\n${list(profile.notes)}`);
  if (profile.critical_paths_matched?.length) sections.push(`### Critical paths touched\n\n${list(profile.critical_paths_matched.map((file) => `\`${file}\``))}`);
  if (profile.warnings.length) sections.push(`### Profile warnings\n\n${list(profile.warnings)}`);
...
export function renderQualityReview(record: RunRecord): string {
  const profile = record.project_profile;
  const quality = profile.loaded ? profile.quality : undefined;
  const preferences = quality ? [
    quality.max_file_lines ? `max file lines ${quality.max_file_lines}` : "",
    quality.max_function_lines ? `max function lines ${quality.max_function_lines}` : "",
    quality.max_line_length ? `max line length ${quality.max_line_length}` : "",
    quality.allow_todos ? "TODO/FIXME/HACK markers allowed" : "",
  ].filter(Boolean) : [];
  const criticalFindings = record.quality_review.findings.filter((item) => (profile.critical_paths_matched ?? []).includes(item.file));
  const preferenceContext = preferences.length ? `- Profile quality preferences applied: ${preferences.join(", ")}\n\n` : "";
  const criticalContext = criticalFindings.length ? `## Findings in Critical Paths\n\n${list(criticalFindings.map((item) => `\`${item.file}\` — ${item.category} (${item.severity})`))}\n\n` : "";
  const context = `${preferenceContext}${criticalContext}`;
  return renderQualityReviewBase(record)
    .replace("## Summary\n\n", `## Summary\n\n- Run ID: ${record.run_id}\n- Created at: ${record.created_at}\n`)
    .replace("## Heuristic Findings", `${context}## Heuristic Findings`);
}
