...
describe("handoff quality output", () => {
  it("writes quality review to the run and latest bundles", async () => {
    ...
    const files = { "HANDOFF.md": renderHandoff(record), "QA_REPORT.md": renderQaReport(record), "AGENT_HANDOFF.md": renderAgentHandoff(record), "QUALITY_REVIEW.md": renderQualityReview(record), "RUN_COMPARISON.md": renderRunComparison(record), "POLICY_REPORT.md": renderPolicyReport(record), "RUN_RECORD.json": `${JSON.stringify(record, null, 2)}\n` };
    ...
    const latestComparison = await readFile(path.join(root, ".relaypoint", "latest", "RUN_COMPARISON.md"), "utf8");
    const latestPolicy = await readFile(path.join(root, ".relaypoint", "latest", "POLICY_REPORT.md"), "utf8");
    ...
    expect(latestComparison).toContain("No previous Relaypoint run was found.");
    expect(latestPolicy).toContain("# Policy Report");
    ...
    expect(json.outputs.run_comparison).toBe("RUN_COMPARISON.md");
    expect(json.outputs.policy_report).toBe("POLICY_REPORT.md");
    expect(json.policy).toMatchObject({ enabled: true, using_defaults: true });
